 ...
 *
 * So then, we make the following shortcut/assumption here in endian_float.h:
...
/*
 * Unlike cpu_to_be32, we will offer no cpu_to_float_be32. This just isn't
 * something that should be passed around in memory.
 */
...
LIBHX_DBG_INLINE void LIBHX_OPT_O2 cpu_to_float_be32p(void *p, float v)
{
	uint32_t w;
#ifdef __cplusplus
	static_assert(sizeof(v) == sizeof(w));
...
LIBHX_DBG_INLINE void LIBHX_OPT_O2 cpu_to_float_le32p(void *p, float v)
{
	uint32_t w;
#ifdef __cplusplus
	static_assert(sizeof(v) == sizeof(w));
...
LIBHX_DBG_INLINE void LIBHX_OPT_O2 cpu_to_float_be64p(void *p, double v)
{
	uint64_t w;
#ifdef __cplusplus
	static_assert(sizeof(v) == sizeof(w));
...
LIBHX_DBG_INLINE void LIBHX_OPT_O2 cpu_to_float_le64p(void *p, double v)
{
	uint64_t w;
#ifdef __cplusplus
	static_assert(sizeof(v) == sizeof(w));
